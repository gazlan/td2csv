...
{
   const char pszCopyright[] = "-*-   td2csv 1.0  *   Copyright (c) Gazlan, 2015   -*-";
   const char pszDescript [] = "TECDOC_CD_3_2015 tables to .CSV converter";
   const char pszE_Mail   [] = "complains_n_suggestions direct to gazlan@yandex.ru";

   printf("%s\n\n",pszCopyright);
   printf("%s\n\n",pszDescript);
   printf("Usage: td2csv.com TableName\n");
   printf("%s\n",pszE_Mail);
}
...
{           
   ...
   Columnnumber      iCols = 0; 
   int               iRow  = 0;

   Int2              Indicator = 0;

   TBEerror(TCIGetResultSetAttributeA(pRes,TCI_ATTR_COLUMN_COUNT,1,&iCols,sizeof(iCols),NULL));
   printf("iCols: %d\n\n",iCols);

   TCIState    Err;

   // 1. Name
   TBEerror(TCIBindColumnA(pRes,1,&tdSysTable._pszName,sizeof(Int4),NULL,TCI_C_SCHARPTR,NULL));
   // 2. Owner
   TBEerror(TCIBindColumnA(pRes,2,&tdSysTable._iOwner,sizeof(Int4),NULL,TCI_C_INT4,NULL));
   // 3. Type
   TBEerror(TCIBindColumnA(pRes,3,&tdSysTable._pszType,sizeof(Int4),NULL,TCI_C_SCHARPTR,NULL));
   // 4. SegNo
   TBEerror(TCIBindColumnA(pRes,4,&tdSysTable._iSegNo,sizeof(Int4),NULL,TCI_C_INT4,NULL));
   // 5. ColNo
   TBEerror(TCIBindColumnA(pRes,5,&tdSysTable._iColNo,sizeof(Int4),NULL,TCI_C_INT4,NULL));
   // 6. Date
   TBEerror(TCIBindColumnA(pRes,6,&tdSysTable._Date,sizeof(tdSysTable._Date),NULL,TCI_C_TIMESTAMP,NULL));
   // 7. Cluster
   TBEerror(TCIBindColumnA(pRes,7,&tdSysTable._iCluster,sizeof(Int2),NULL,TCI_C_INT2,NULL));
   // 8. Version
   TBEerror(TCIBindColumnA(pRes,8,&tdSysTable._iVersion,sizeof(Int4),NULL,TCI_C_INT4,NULL));
   // 9. IndexType
   ...
   { 
      ...
      // 6. Date
      fprintf(_pOut,", %lu",tdSysTable._Date);
      // 7. Cluster
...
//////////////////////////////////////////////////////////////////////////
   ...
   { 
      ...
      TBEerror(TCIGetResultSetAttributeA(pRes,TCI_ATTR_COLUMN_TYPE,ii + 1,&dt,sizeof(dt),NULL));

      // do something with the type, e.g. switch over it and allocate space for the result
      switch (dt) 
      { 
...
/*
            ...
            TBEerror(TCIGetResultSetAttributeA(pRes,TCI_ATTR_COLUMN_PRECISION,4,&prec, sizeof(prec), NULL));
            TBEerror(TCIGetResultSetAttributeA(pRes,TCI_ATTR_COLUMN_SCALE,    4,&scale,sizeof(scale),NULL));

            fprintf(_pOut,"prec : %d\n",prec);
            fprintf(_pOut,"scale: %d\n",scale);
*/
            break; 
         }
      } 
